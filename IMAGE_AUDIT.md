...
## R22D -- image-pair panorama stitching

CrossEngin's CV pipeline already lands feature DETECTION + MATCHING
(R5C SIFT 128-D + Lowe ratio test, R6D ORB 256-bit BRIEF + Hamming
ratio test) and structural extractors (Harris corners, Sobel/Canny
edges, HOG dense descriptor + sliding-window detector, LBP texture,
spatial k-means segmentation, SLIC superpixels). What was missing
was a downstream APPLICATION the matched features feed into -- a
module that CONSUMES the keypoint correspondences and does something
visible with them. Panorama stitching is the classical CV recipe
that lives on top of feature matching, and R22D ships the single-
pair version of this recipe.

### Algorithm (Brown & Lowe 2007, single-pair planar variant)

Given two overlapping PGM images A and B of equal size (w x h):

1. **Feature matching.** Run ORB (default) or SIFT on both images;
   produce a list of `[xa, ya, xb, yb]` correspondences where
   (xa, ya) in A maps to (xb, yb) in B.

2. **RANSAC homography estimation.** Repeatedly sample 4 random
   correspondences, fit the candidate 3x3 homography via the
   Direct Linear Transform (DLT) on an 8x9 augmented matrix with
   h33 fixed, count inliers (correspondences within
   `threshold_px` reprojection distance), keep the best.

3. **Backward warp + bilinear sampling.** Allocate the output
   canvas. For every output pixel `(xo, yo)`, compute the source
   coordinate `(xs, ys)` via the inverse homography, and bilinear-
   sample image B at the fractional source coordinate. Out-of-
   bounds source coordinates emit 0 (black).

4. **Linear blend.** Average the overlap region 50/50 with image A
   in its original frame; pass through the non-overlap regions
   unchanged.

### Pixel-area cap

256 per axis on inputs (matches R5C SIFT + R6D ORB caps); 512 per
axis on the output canvas (a side-by-side mosaic of two 256-wide
inputs with a 10-pixel overlap caps at 502 px, comfortably below
the 512 ceiling). RANSAC iterations capped at 500; reprojection
threshold capped at 20 px.

### Why ORB by default (and SIFT as a flag)

R6D ORB is integer-only (FAST + rotated BRIEF + Hamming distance);
its descriptor is 256 bits and the matcher uses popcount-XOR.
R5C SIFT runs slower per descriptor (128 ints, L2 distance, ratio
test) but recovers more keypoints on textured scenes. The default
is ORB for speed; pass `PANO_USE_SIFT` (=2) as the `use_orb_or_sift`
argument to `pano_match_features` for SIFT instead.

### Public API

* `pano_match_features(image_a, image_b, w, h, use_orb_or_sift) ->
  list[match_pair]` -- detect + describe + ratio-test match across
  both images; re-emits correspondences as `[xa, ya, xb, yb]`
  tuples in original image coordinates.

* `pano_ransac_homography(matches, threshold_px, max_iterations)
  -> [H_milli_9, inlier_count]` -- DLT-based RANSAC homography
  estimator. `H_milli_9` is a 9-int row-major 3x3 matrix in
  milli-fixed-point (identity is
  `[1000, 0, 0, 0, 1000, 0, 0, 0, 1000]`). LCG-deterministic
  sampling: same input -> same output across runs.

* `pano_warp(image, w, h, H_milli, out_w, out_h) -> byte_buffer_ptr`
  -- backward warp via the inverse of `H_milli` + bilinear sampling.
  Returns `out_w * out_h` raw grayscale bytes (caller frees).

* `pano_blend(image_a, image_b_warped, w, h) -> pgm_bytes_ptr` --
  produces PGM-P5 bytes of the linearly blended pair.

* `pano_stitch(image_a, image_b, w, h) -> pgm_bytes_ptr` -- the
  convenience full-pipeline wrapper. Sets up the
  `(2*w - PANO_DEFAULT_OVERLAP_PX)`-wide canvas, runs matching +
  RANSAC + warp + blend, returns PGM-P5 bytes. Graceful fallback
  to a translation when feature matching produces < 4 valid pairs
  (so uniform-image inputs still produce a recognisable mosaic).

* Auxiliary: `pano_homography_identity`, `pano_homography_translation`,
  `pano_homography_invert`, `pano_homography_apply`,
  `pano_inlier_count`, `pano_homography`, `pano_pgm_size`,
  `pano_save_pgm`, `pano_pgm_args` (the chat admin entry point).

### Honest scope (R22D.2 follow-ups)

Real panorama benchmarks use bundle adjustment + multi-band blending
+ cylindrical / spherical warping. R22D ships the SINGLE-PAIR
homography + LINEAR blend -- the core pipeline. Multi-image
panoramas (N > 2) and multi-band blending are R22D.2 follow-ups.

The DLT solver uses milli-fixed-point Gaussian elimination, which
...
Unit tests (~59 assertions in `tests/unit/test_image_panorama.nova`,
NEW): identity homography shape + apply, translation homography
apply, invert round-trip on translation, identity warp preserves
image, translation warp shifts pixel, warp invalid-input rejection,
RANSAC with 4 inliers + 0 outliers recovers exact H within 1 px,
RANSAC with 4 inliers + 4 outliers rejects outliers, RANSAC < 4
matches returns identity, uniform-image match collapse, match
invalid-input rejection, blend A-only region passes A through,
blend 50/50 overlap, blend B-only region passes B through,
uniform-image stitch returns valid PGM, checkerboard-halves
stitch covers full canvas width with non-zero pixels at both
edges, match-pair accessors round-trip, PGM size = header + area.

Integration scenario `tests/integration/scenario_iiii_panorama.sh`
(17 assertions, NEW): standalone NOVA driver synthesizes a 36x32
checkerboard pair with 10-pixel overlap, drives `/pano` through
the chat, verifies dimensions + matches count + output path +
stitched.pgm exists + size > 1024 bytes + P5 magic header + 62x32
output dims; `/pano` with no arg prints usage; `/pano` on missing
file emits graceful FAILED. Passes when every assertion holds.

All prior CV tests stay green (R5C SIFT 25, R6D ORB 34, R14D HOG
55, R15C HOG detector 32, R16D face_detect 36, R17D LBP 45, R21D
HOG integral 42). Module count: +1
(`src/io/transducers/image_panorama.nova` NEW).

### Files touched / added

* `src/io/transducers/image_panorama.nova` -- NEW (~720 lines:
  module docs + format / cap constants + match-pair record slot
  indices + result-tuple slot indices + `pano_homography_identity`
  / `pano_homography_translation` / `pano_homography_apply` /
  `pano_homography_invert` + `pano_match_features` (ORB + SIFT
  paths) + LCG sampler + `_pano_solve_homography_dlt` (DLT) +
  `_pano_count_inliers` + `pano_ransac_homography` +
  `_pano_bilinear_sample` + `pano_warp` (backward warp + bilinear)
  + `pano_blend` (PGM-P5 emitter) + `pano_stitch` (full pipeline
  + fallback translation when matches < 4) + PGM-P5 header writer
  helpers + `pano_pgm_size` + `pano_save_pgm` + `pano_pgm_args`
  (chat admin entry)).
* `tests/unit/test_image_panorama.nova` -- NEW (18 test functions
  / 59 assertions).
* `tests/integration/scenario_iiii_panorama.sh` -- NEW (17
  assertions; chat-driven end-to-end check on a synthesized
