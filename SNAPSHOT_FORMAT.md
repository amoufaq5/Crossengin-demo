...
## Ed25519 signature on the Merkle root (R16A)

R15E's Merkle root closes the "single-byte tamper" gap because the
recompute-vs-claim equality fails. It does NOT close the
"attacker-controls-the-whole-file" gap — such an attacker can tamper
an atom AND rewrite the meta.merkle_root line so the equality still
holds (both sides become the attacker-chosen value).

R16A binds the root to an offline long-term Ed25519 key. The signer
holds the 32-byte private seed; the verifier holds the 32-byte
public key out-of-band (typed in, copied via env var, or read from
a protected file). The substrate stores no key material in the
snapshot file itself; only the signature.
...
bytes** (NOT the 64-char hex rendering). Two reasons: Ed25519's
output is deterministic, so signing the same root twice produces
the same signature — a property snapshot determinism (two `/save`
calls on an unchanged KG produce bit-identical files) inherits; and
signing the raw bytes is the standard shape, not tied to one text
representation.

A pre-R16A file omits this line; the verifier treats absence as
"no signature emitted by this writer" — the same shape R15E used
for the merkle_root slot.

### Keypair on-disk format

The simplest format that round-trips:

```
<base>.priv  -- 32 raw bytes (the Ed25519 seed; mode 0600 = rw-------)
<base>.pub   -- 32 raw bytes (the Ed25519 pubkey; mode 0644 = rw-r--r--)
```

We store the **seed** (not the derived scalar) for the private file
because that's what `ed25519_sign` takes as input. Both files are
**exactly 32 bytes** — a trailing newline / hex / PEM wrap would
force per-format parsing.

Generated by `examples/snap_keygen.nova` (run via `CE_SNAP_KEY_BASE=<base>
$NOVA_ROOT/nova run examples/snap_keygen.nova`). Higher-level operators
may layer PEM / age / yubikey on top — those are orthogonal to the
substrate's attestation contract.

### Env-var contract

| Variable                          | When set                          | Effect                                                  |
|-----------------------------------|-----------------------------------|---------------------------------------------------------|
| `CE_SNAPSHOT_SIGN_KEY=<priv>`     | On `/save`                        | Reads `<priv>.priv` + `<priv>.pub`, signs the recomputed Merkle root, emits `meta.merkle_signature`. |
| `CE_SNAPSHOT_VERIFY_PUBKEY=<pub>` | On `/load` and `/snap_sign_status` | Reads the 32-byte pubkey file; verifies the signature over the recomputed Merkle root; refuses the load on mismatch. |
| `CE_SNAPSHOT_REQUIRE_SIGNATURE=1` | On `/load`                        | Strict mode: an unsigned snapshot with verify-pubkey set is REFUSED. Default = lenient (warn + proceed). |

The signer reads the priv on every save (instead of holding it in
memory) so the blast radius of a memory disclosure between writes
stays minimal.

### Verification surfaces

* **`/snap_sign_status [PATH]`** — chat command that reports whether
  the file has a signature, whether `CE_SNAPSHOT_VERIFY_PUBKEY` is
  set, and the last verify result (`verified | TAMPERED | no_signature
  | no_pubkey | file_missing`).
* **`snap_load(path)` with `CE_SNAPSHOT_VERIFY_PUBKEY=<pub>`** — the
  normal rehydrate path becomes a tripwire. A mismatch returns 0
  (the same sentinel a parse failure uses) and prints a
  `(load FAILED: Merkle signature mismatch ...)` line. Combined with
  `CE_SNAPSHOT_VERIFY_MERKLE=1` (R15E), the substrate gets full
  attestation: Merkle root commits to KG contents, signature commits
  to Merkle root.
* **`snap_verify_signature(s)` / `snap_sign_status_for_path(path)`**
  — module-level APIs for federation-peer attestation surfaces.

### Backwards-compat policy

The R16A meta line is OPTIONAL. A signed snapshot (R16A writer) is
forward-compat with pre-R16A readers (they ignore the line). An
unsigned snapshot (pre-R16A writer or R16A writer without the env
var) loads cleanly under any reader unless
`CE_SNAPSHOT_REQUIRE_SIGNATURE=1` is set, which strict-flips it.
This mirrors the R15E "no commitment, no check" branch and the
existing `CE_SNAPSHOT_*` opt-in pattern.

### Performance

One `ed25519_sign` per `/save` when the env var is set
(~250-1100 ms on the sandbox per `src/safety/ed25519.nova`'s
preamble, measured at ~241 ms in the R16A integration scenario).
One `ed25519_verify` per `/load` when the verify env var is set.
Both are one-shot per snapshot — not per-atom — so the cost is
linear in saves/loads, not in KG size.

### See also

* `src/persistence/merkle_signing.nova` — the R16A module:
  `merkle_sign`, `merkle_verify_signature`, `merkle_sign_hex`,
  `merkle_verify_signature_hex`, `merkle_signing_keypair_save`,
  `merkle_signing_keypair_load`, env-var hooks.
* `src/persistence/snapshot_writer.nova` — `SNAP_META_MERKLE_SIGNATURE`
  slot, `snap_meta_set_merkle_signature`, `snap_meta_has_merkle_signature`.
* `src/persistence/snapshot_disk.nova` — `snap_verify_signature`,
  `snap_sign_status_for_path`, the `snap_load` tripwire.
* `tests/unit/test_merkle_signing.nova` — 51 assertions covering
  the sign / verify primitives, hex codec, determinism, tamper
  detection (sig / pubkey / root flips), keypair save+load
  round-trip, env-var lookup path.
* `tests/integration/scenario_mmm_merkle_signing.sh` — end-to-end
  test: keygen → signed /save → meta line present → /snap_sign_status
  reports verified → determinism (two signatures bit-identical) →
  tamper detection via verify-pubkey on /load → strict mode
  REFUSES an unsigned file.
* `examples/snap_keygen.nova` — keypair generator.
