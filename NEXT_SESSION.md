...
cryptographically broken... but RFC 5389 §15.4 mandates it for the
long-term-credential mechanism. R36A remains on-spec; SCRAM-SHA1 /
SCRAM-SHA256 (RFC 7635) is the modern replacement and is deferred").
R38D ships SCRAM-SHA-256 alongside MD5 -- both auth paths coexist;
clients negotiate via the new SECURITY-FEATURES attribute.

### What R38D delivers

* **`src/safety/scram.nova`** (NEW, ~600 lines + 130-line header) --
  the SCRAM primitives leaf module:
  * `pbkdf2_hmac_sha256(pwd, pwd_n, salt, salt_n, iter, dkLen) ->
    buf` -- RFC 8018 §5.2 F-loop (`U_1 = HMAC(pwd, salt || INT(i))`;
    `U_j = HMAC(pwd, U_{j-1})`; output = XOR of all U_j).
  * `scram_base64_encode(buf, n) -> string` /
    `scram_base64_decode(s) -> [buf, n] | 0` -- RFC 4648 §4
    standard alphabet with '=' padding; inline (no separate
    `safety/base64.nova` -- future R38D.2 can extract).
  * Four RFC 5802 §5.1 message constructors:
    `scram_client_first(user, nonce)`,
    `scram_server_first(c_nonce, s_nonce, salt_b64, iter)`,
    `scram_client_final(cb_b64, nonce, proof_b64)`,
    `scram_server_final(sig_b64)`. Plus
    `scram_auth_message(cfb, sf, cfnp)` for the AuthMessage
    concatenation per RFC 5802 §3.
  * Six SCRAM proof / signature primitives:
    `scram_salted_password(pwd, salt, salt_n, iter)`,
    `scram_client_key(sp)`, `scram_stored_key(ck)`,
    `scram_server_key(sp)`, `scram_client_proof(sp, am, am_n)`,
    `scram_server_signature(sp, am, am_n)`.
  * Two convenience wrappers:
    `scram_build_client_final(user, pwd, c_nonce, s_nonce, salt_b64,
    iter) -> "c=biws,r=NONCE,p=PROOF"` and
    `scram_compute_server_signature_b64(...)` for server-final
    verification.
  * `scram_nonce_from_counter(counter, n)` -- deterministic
    counter+SHA-256 nonce for testability.
  * `scram_random_nonce(n)` -- production CSPRNG stub that returns
    0 to force callers through an OS-entropy seam they must wire
    (NOVA's runtime does not currently expose `sys_getrandom`;
    deterministic helper is the test-time path).

* **`src/federation/turn.nova` extension (PURELY ADDITIVE)** --
  R34B / R35B / R36A exports preserved BYTE-IDENTICAL. New surface:
  * `TURN_ATTR_USERHASH = 0x001E` (RFC 7635 §3.1.1)
  * `TURN_ATTR_MESSAGE_INTEGRITY_SHA256 = 0x001C` (RFC 7635 §3.2)
  * `TURN_ATTR_SECURITY_FEATURES = 0x802A` (RFC 7635 §4)
  * SECURITY-FEATURES flag bits: `TURN_SECFEAT_PWD = 1` (MD5),
    `TURN_SECFEAT_SCRAM = 2` (SCRAM-SHA-256),
    `TURN_SECFEAT_TLS_BIND = 4` (reserved -- not implemented).
  * `turn_userhash(user, realm) -> 32B buf` = SHA-256(user + ":"
    + realm) per RFC 7635 §3.1.1.
  * `turn_negotiate_security_features(server_flags) -> client_flags`
    -- "pick strongest available"; SCRAM wins over PWD when both
    advertised; falls back to PWD otherwise.
  * `turn_emit_allocate_request_scram(txn, lifetime, transport,
    user, password, realm, c_nonce, s_nonce, salt_b64, iter) ->
    [buf, n]` -- the SCRAM-authed Allocate Request:
    `LIFETIME + REQUESTED-TRANSPORT + USERNAME + USERHASH + REALM +
    NONCE + SECURITY-FEATURES + MESSAGE-INTEGRITY-SHA256` (the
    NONCE attr carries the SCRAM server-first text
    `"r=COMBINED,s=SALT,i=ITER"` so the client + server compute the
    same AuthMessage). MI-SHA-256 key = SCRAM `StoredKey` =
    SHA-256(HMAC(PBKDF2(password, salt, iter, 32), "Client Key"));
    MAC computed over the prefix `[0..MI_attr_start)` per the
    RFC 5389 §15.4 + RFC 7635 §3.2 rule.
  * `turn_parse_scram_challenge(buf, n) ->
    [combined_nonce_str, salt_b64_str, iter, realm_str] |
    TURN_ERR_*` -- extracts the SCRAM challenge text from a 401
    Unauthorized response's NONCE attribute.
  * `turn_emit_scram_challenge_response(txn, realm, c_nonce,
    s_nonce, salt_b64, iter, server_features) -> [buf, n]` --
    builds a 401 with the canonical RFC 5802 §5.1 server-first
    text (test-facing; a future server-side validator can lift it).
  * `turn_verify_message_integrity_sha256(buf, n, stored_key) -> 1 | 0`
    -- verifies the MI-SHA-256 attribute against a re-derived
    StoredKey.

### Verification

* `tests/unit/test_scram.nova` (NEW) -- **43 assertions**, all
  passing. RFC 4648 §10 base64 vectors, binary round-trip,
  invalid-length rejection, alphabet boundaries; RFC 7914 §11
  PBKDF2-HMAC-SHA256 at c=1 dkLen=64 (the canonical 128-hex-char
  known output) + dkLen=32 truncation check + c=0 rejection;
  SCRAM key schedule shape (ClientKey != ServerKey, StoredKey
  deterministic, ClientProof XOR ClientSignature = ClientKey);
  the headline RFC 7677 §3 SCRAM-SHA-256 canonical round-trip at
  c=4096 (~16K SHA-256 compressions / ~100ms in NOVA bytecode)
  ...
  vectors; the `scram_build_client_final` wrapper emits the
  complete `"c=biws,r=...,p=..."` line per RFC 5802 §5.1; SCRAM
  message constructors round-trip; nonce determinism (same
  counter -> same nonce; different counter -> different nonce);
  CSPRNG stub returns 0.

* `tests/unit/test_turn.nova` -- **46 new assertions** (turn:
  OK 451 checks, vs R36A's 405 baseline; **405 prior assertions
  BYTE-IDENTICAL** per the additive-only contract). New SCRAM
  block covers: attribute constants; USERHASH matches direct
  SHA-256; negotiation prefers SCRAM, falls back to MD5 when only
  PWD or TLS-binding advertised; SCRAM challenge response emit +
  parse round-trip (combined_nonce / salt_b64 / iter / realm
  extracted from NONCE attr text); SCRAM challenge parse rejects
  non-401 codes; field-extraction helper; SCRAM Allocate Request
  walks back to 8 attributes in the canonical order with the
  expected lengths; MI-SHA-256 self-verify succeeds with the
...
* 142 prior `test_ice_turn.nova` assertions BYTE-IDENTICAL.
  R38C's 111 `test_turn_server.nova` assertions BYTE-IDENTICAL
  (R38C's read-only turn.nova import remains compatible).
  All R38D commits touch ONLY R38D-owned paths.

### Honest design caveats

* **PBKDF2 at the RFC 5802 §5.1 minimum of 4096 iterations is
  SLOW** (~16K SHA-256 compressions per derive). The canonical
  RFC 7677 §3 round-trip runs in ~100ms in the NOVA bytecode
  runtime. A real client SHOULD cache `SaltedPassword` keyed on
  (password, salt, iter); R38D does NOT ship that cache. A future
  R38D.2 can add a SaltedPassword cache slot to the client state
  (mirroring R36B's DTLS-SRTP keying material cache).

* **Base64 helpers are INLINE in scram.nova** rather than extracted
  to `src/safety/base64.nova`. The brief allows this for the
  current round; a future R38D.2 could extract alongside an
  x509 BER consumer that needs base64 of its own (DER-from-PEM).

* **CSPRNG nonce is a STUB** -- `scram_random_nonce(n)` returns
  0. The deterministic `scram_nonce_from_counter` is the test-time
  path; it is NOT secret-sharing-safe (same counter -> same nonce
  -> catastrophic for prod). NOVA's runtime needs to expose an
  OS-entropy primitive before production nonce generation is
  wired.

* **TLS channel binding (RFC 7635 §3 + RFC 5929) is NOT shipped**
  even though `TURN_SECFEAT_TLS_BIND` is defined. The SCRAM `c=`
  attribute is hard-coded to "biws" (no binding). Adding binding
  requires a hash of the TLS server cert which lives in R31B's
  DTLS state -- future R38D.2.

* **R38D verification surface is the CLIENT side.** R38D ships
  `turn_verify_message_integrity_sha256` for server-side
  validators to use, but does NOT ship a live SCRAM auth flow
  on the SERVER side of turn_server.nova (R38C's territory). A
  future round can layer the server-side validator on top.

### Files touched

* NEW: `src/safety/scram.nova`, `tests/unit/test_scram.nova`
* EXTEND (additive only): `src/federation/turn.nova`,
  `tests/unit/test_turn.nova`
* DOCS: `NEXT_SESSION.md` (this section), `FEDERATED_AUDIT.md`
  (R38D extension), `README.md` (R38D banner + status-table cell)

R38B (DTLS PRF rekey, `2344514`) and R38C (TURN server-side state
machine, `1bea868`) landed on the branch during R38D's session;
both are unrelated to R38D's surface and remain untouched.
...
R34B / R35B / R35D in one bundle: long-term-credential authentication
(RFC 5389 §10 / RFC 5766 §4), per-permission refresh cadence
(RFC 5766 §8 / §11), and a permission-lifetime override hook.

### What R36A delivers

* **Inline MD5 (RFC 1321) + HMAC-SHA1 (RFC 2104 + RFC 3174)** local
  to `src/federation/turn.nova`. ~80 lines MD5 + ~120 lines SHA-1 +
  ~50 lines HMAC-SHA1 wrapper. Re-inlined (rather than imported)
  because R34C ships `_srtp_hmac_sha1` under an underscore-prefixed
  (private) name; un-mangling on R34C's side would break a sealed
  module. A future R36A.2 can canonicalize MD5 + SHA-1 into
  `safety/md5.nova` + `safety/sha1.nova` alongside the existing
  `safety/sha256.nova` (R33A/R34A).
* **`turn_hmac_sha1_key(username, realm, password) -> 16-byte buf`**
  computes `MD5("username:realm:password")` per RFC 5389 §15.4
  ("key" for HMAC-SHA1 over messages).
* **`turn_emit_allocate_request_authed(txn, lifetime, transport,
  username, password, realm, nonce) -> [buf, n]`** emits an
  Allocate Request with USERNAME + REALM + NONCE attributes followed
  by MESSAGE-INTEGRITY. The length field in the STUN header carries
  the FULL body length (including the MI attribute's 24 bytes), but
  the HMAC is computed over the bytes `[0..MI_attr_start)` -- RFC
  5389 §15.4 spec rule. Verified by recomputing the HMAC in tests
  and confirming equality with the embedded value.
* **`turn_parse_401_response(buf, n) -> [realm_buf, realm_n,
  nonce_buf, nonce_n] | TURN_ERR_*`** extracts the auth challenge
  from a 401 Unauthorized Allocate Error response. Rejects responses
  whose ERROR-CODE is non-401 (`TURN_ERR_METHOD`) or missing REALM /
  NONCE (`TURN_ERR_NO_ATTR`).
* **`turn_verify_message_integrity(buf, n, key) -> 1 | 0`**
  client-side verification of server responses that include MI.
  Returns 0 (no crash) when the message has no MI attribute --
  documented tolerance for servers that omit MI on success.
* **`TURN_CHANNEL_LIFETIME_DEFAULT = 600`** matches RFC 5766 §11.
  Re-pins `TURN_PERM_LIFETIME_DEFAULT = 300` from R35B.
* **`turn_client_tick_perms(state, current_time_unix) ->
  [n_perms_expired, n_channels_expired]`** walks
  `permissions_list` + `channels_list` and `list_remove`s entries
  whose `expiry < current_time_unix`. Bumps `stats_perm_expired`
  + `stats_channel_expired` counters (new state slots). Walks
  back-to-front so removal does not shift the live cursor.
* **`turn_client_tick_authed(state, current_time_unix)`** is the
  composed alloc-expiry + per-permission-cadence tick that callers
  drop in to replace R35B's `turn_client_tick`. Same return tag
  contract (`TURN_TICK_OK` / `TURN_TICK_EXPIRED` for the alloc
  expiry; perm pruning is a side-effect).
* **`turn_set_perm_lifetime_default(state, secs)`** lets callers
  configure a non-300s default for non-RFC servers. The value is
  stored in `_TURN_ST_PERM_LIFETIME_DEFAULT` (slot 24) and consulted
  by `turn_client_restamp_last_permission(state, now)` which
  re-stamps the last appended permission record's expiry. Because
  CreatePermission success responses do NOT echo a lifetime
  (unlike Allocate which echoes LIFETIME), this is the cleanest
  hook -- callers can override and re-stamp after the standard
  R35B recv path.
* **`ice_turn_handle_allocate_response_authed(state, recv_buf, n)`**
  in `src/federation/ice_turn.nova`. Parallel handler that
  parses the response with `turn_parse_401_response` FIRST -- on
  401, transitions to a new `ICE_TURN_AUTH_PENDING` state, stashes
  REALM + NONCE as freshly-alloc'd copies (so they outlive
  `recv_buf`), and returns `ICE_TURN_RELAY_AUTH_PENDING`. Non-401
  responses fall through to R35D's `ice_turn_handle_allocate_response`
  unchanged.
* **`ice_turn_credentials(state, username, password) ->
  [emit_buf, n] | 0`** -- caller hands in the creds after receiving
  AUTH_PENDING. We compose the stashed REALM + NONCE with the
  caller's creds via `turn_emit_allocate_request_authed` and bump
  `stats_auth_retries`. Refuses to emit (returns 0) if `auth_state
  != PENDING`.
...
* **New state slots appended (preserving the R35D 17-slot +
  R35B 24-slot layouts byte-for-byte)**: in turn.nova,
  `_TURN_ST_PERM_LIFETIME_DEFAULT` (24), `_TURN_ST_STATS_PERM_EXPIRED`
  (25), `_TURN_ST_STATS_CHAN_EXPIRED` (26); in ice_turn.nova,
  `_ICE_TURN_S_AUTH_REALM` (17), `_AUTH_REALM_N` (18),
  `_AUTH_NONCE` (19), `_AUTH_NONCE_N` (20), `_AUTH_STATE` (21),
  `_STATS_AUTH_RETRIES` (22). `turn_client_extend_state_r36a` /
  `ice_turn_extend_state_r36a` lazily extend in-place so R35B /
  R35D states keep working through the new accessors.

### Verification

* **`tests/unit/test_turn.nova`** -- 82 new R36A assertions
  appended to the tail of the suite. **323 prior assertions
  byte-identical** (lines 1..1366 unchanged). New coverage: MD5
  RFC 1321 §A.5 vectors (empty / `"abc"` / `"message digest"`),
  HMAC-SHA1 RFC 2202 TC1 + TC2, `turn_hmac_sha1_key` matches
  direct MD5, authed Allocate attribute-order classification,
  MI byte-position rule, self-emit MI verify, wrong-key MI fails,
  no-MI message returns 0 (no crash), `parse_401` happy path /
  no-REALM rejection / non-401 rejection, lifetime constants,
  `tick_perms` removes-when-expired / keeps-in-window / mixed
  window, channel pruning, composed `tick_authed`, lifetime
  override reflected after recv, no-alloc tick is `[0, 0]`,
  R35B-shape state auto-extends to 27 slots.
* **`tests/unit/test_ice_turn.nova`** -- 54 new R36A assertions
  appended to the tail of the suite. **88 prior assertions
  byte-identical** (lines 1..545 unchanged). New coverage:
  `init_authed` shape (23 slots, all auth fields zero), 401 ->
  AUTH_PENDING (REALM + NONCE stashed as freshly-alloc'd copies),
  credentialed re-emit is a real Allocate Request with 6 attrs
  ending in MI, credentialed re-emit success -> ACTIVE + relay
  candidate injected, second 401 -> RELAY_FAILED + AUTH_FAILED,
  non-401 errors fall through to the R35D handler unchanged,
  `credentials()` refused when not PENDING, first-try success
  skips the auth path, full end-to-end (gather host -> all pairs
  FAIL -> escalate -> 401 -> credentials -> success -> relay
  candidate present), counter does not bump on refused
  `credentials()` call.
* **Suite tally**: turn `OK (405 checks)` + ice_turn `OK (142
  checks)` -- 411 + 136 = 547 total federation assertions across
  the two suites.

### Deferred to future rounds

* **SCRAM-SHA1 / SCRAM-SHA256 (RFC 7635)**. The modern replacement
  for the long-term-credential MD5(user:realm:pass) scheme.
  Out of scope for R36A which targets RFC 5389 §15.4 compliance.
* **MD5 / SHA-1 canonicalization** under `src/safety/md5.nova` +
  `src/safety/sha1.nova` (parallel to R33A's `safety/sha256.nova`
  dedup). R36A inlines locally to keep turn.nova self-contained;
  R36A.2 can re-route turn.nova + srtp.nova through the canonical
  modules without touching either consumer's wire format.
* **txn id rotation between the initial Allocate and the
  credentialed retry.** R36A re-uses the same txn id (kept simple;
  the response correlator naturally accepts the retry). A future
  refinement could rotate to a freshly random txn id and stash both
  IDs so the response handler matches either.
* **CreatePermission / ChannelBind with USERNAME + MESSAGE-INTEGRITY**.
  RFC 5766 §9 / §11 say authenticated allocations MUST attach the
  auth attrs to every subsequent request. R36A ships the
  authentication primitives but does NOT yet thread them through
  R35B's `turn_client_send_permission` / `_channel_bind`. A future
  R36A.3 can stamp the auth attrs on those request paths.
* **MI-bearing success responses**. R36A's
  `turn_verify_message_integrity` accepts and verifies them, but
  R35B's `turn_client_recv` does NOT call the verifier on incoming
  success responses. Callers can verify manually; the integration
  is deferred so the existing recv path stays at its current
  complexity.

### Honest design caveats

* **MD5 is cryptographically broken** for collision resistance
  (Wang 2004, FastColl 2007, MD5SHATTERED-style chosen-prefix
  2009). RFC 5389 §15.4 nevertheless MANDATES
  `MD5(user:realm:pass)` as the HMAC-SHA1 key for the
  long-term-credential mechanism, so R36A inlines MD5 to remain
  on-spec. Callers concerned about collision attacks should
  migrate to SCRAM-SHA1 / SCRAM-SHA256 (RFC 7635) when servers
  support it.
* **MD5 IS A THIRD INLINE CRYPTO PRIMITIVE** that should
  eventually canonicalize alongside `safety/sha256.nova` (R33A)
  and the in-flight SHA-1 inside `srtp.nova` (R34C). R36A logs
  the dedup deferral in this section.
* **`_srtp_hmac_sha1` from R34C is private** (underscore prefix).
  Rather than break R34C's sealed module by un-mangling on its
  side (out of scope per the R36A brief), R36A re-inlines
  HMAC-SHA1 locally. The two copies remain algorithmically
  identical -- a future round can canonicalize.
* **The retry txn id is NOT rotated.** R36A re-uses the txn id
  from the initial Allocate (keeps response correlation simple).
  A real TURN client may want to rotate -- noted as future work
  above.
* **Channel-binding expiry rule is "lazy"**: R35B's
  `turn_client_send_channel_bind` -> recv path appends a
  3-element `[chan_num, ip, port]` record. R36A ships
  `_turn_record_channel_r36a` that appends a 4-element
  `[chan_num, ip, port, expiry]` and `_turn_channel_expiry()`
  treats 3-element records as "no expiry, never prune". This
  keeps R35B's test assertions byte-identical and lets R36A-aware
  callers opt-in to the 600s default without retro-breaking the
  prior contract.
...
## R36B (R35A.2 / R33B.3) -- cached DTLS-SRTP keying material slot

**Status: complete** -- closes R35A's "future hardening" caveat
that the 60-byte PRF expansion in `dtls_export_srtp_keying_material`
is recomputed on every call. R36B memoizes the buf in a fresh
tail-appended state slot and clears it on `dtls_advance_epoch`.

### What R36B delivers

* **Two new state slots in `src/federation/dtls12.nova`**, appended
  at the tail (slots 0..42 from R29B + R31B + R32B + R33B remain
  byte-identical):
  * `DTLS_S_SLOT_SRTP_KM_CACHED = 43` -- holds `0` (no cache; next
    call computes) or the pointer to the 60-byte PRF expand. The
    cached pointer IS the pointer returned to all callers (not a
    fresh copy each time) so callers can pin pointer equality.
  * `DTLS_S_SLOT_STATS_SRTP_KM_HITS = 44` -- cumulative cache-hit
    counter. First compute (miss) does NOT bump; every
    serve-from-cache does. Cumulative across the connection
    lifetime; NOT reset on epoch advance.
* **Extended `dtls_export_srtp_keying_material(state)`**: cache
  fast-path on hit (bump hits, return cached pointer); existing
  PRF compute path on miss + stash the returned pointer in the
  cache slot. The `cipher_active` gate still runs BEFORE the
  cache lookup, so a pre-handshake export still returns 0.
* **Cache invalidation hook** in `dtls_advance_epoch(state)`: one
  new line clears the cache slot to 0. Rationale: RFC 5764 §4.2
  allows a DTLS re-handshake to feed new SRTP keys, and we cannot
  tell from inside `advance_epoch` whether the caller intends a
  full re-handshake or a key_block-only refresh. Safe default
  is to recompute on next export -- if master_secret is genuinely
  unchanged the recompute yields the SAME 60 bytes (idempotent).
* **New accessor `dtls_stats_srtp_km_hits(state)`** -- thin
  one-liner exposing slot 44.
* **`dtls_stats_line(state)` extended** with `srtp_km_hits=<n>`
  appended at the tail of the existing dtls12 stats string. Prior
  R32B / R33B substring-scan assertions are unaffected.

### Verification

* **`tests/unit/test_dtls12.nova`** extended by 32 R36B
  assertions across 8 new test functions, all appended at the
  tail of the suite. Highlights:
  * Init cache slot + hits are 0 (`init_cache_slot_zero`).
  * First call is a miss -- returns 60B, populates slot 43, hits
    stays 0 (`first_call_populates_cache`).
  * Second call returns SAME POINTER as first (POINTER-equal, not
    just byte-equal) and hits bumps to 1
    (`second_call_serves_cached_pointer`).
  * Third call -> hits=2 (`third_call_bumps_hits_to_two`).
  * `dtls_advance_epoch` clears the cache; next export is a miss
    (no hits bump); call after that hits to 3
    (`advance_epoch_invalidates_cache`).
  * Cached bytes byte-identical guard
    (`cached_bytes_match_first_compute`).
  * Per-state cache independence: alice + bob have separate
    caches + hit counters (`caches_are_per_state`).
  * Pre-handshake gate not bypassed by empty cache
    (`cache_does_not_bypass_cipher_active_gate`).
* **369 prior dtls12 assertions byte-identical** -- slots 43+44
  are tail-appended; slots 0..42 untouched; existing tests do
  not index into slot 43+ or expect a specific stats_line shape.

### Deferred to future rounds

* **Manual cache invalidation.** No public
  `dtls_invalidate_srtp_km_cache(state)` function shipped. If a
  future use case needs a manual flush under unchanged epoch,
  add a one-line `state[SRTP_KM_CACHED] = 0` accessor.
* **Constant-time considerations.** Cache-hit latency vs miss
  latency leaks no key bits (it's just `if cache != 0 return`),
  but if a future threat model wants to mask the latency the
  appropriate fix is to revert to per-call recompute (i.e.
  R35A's shape) -- a one-line revert.

### Honest design caveats

* **Invalidate-on-epoch-advance pays one extra PRF expansion per
  plain CCS.** A pure key_block CCS (same master_secret) could
  technically keep the cache valid; we invalidate anyway because
  epoch advance is rare and we cannot tell from
  `dtls_advance_epoch` whether the caller intends a re-handshake
  or a key_block-only refresh. Cost: two HMAC-SHA256 iterations
  per epoch transition. The opposite default (never invalidate)
  would silently serve stale SRTP keys after a real DTLS
  re-handshake -- a security footgun we explicitly reject.
* **Compile/test unverified in this session.** No Nova toolchain
  available in the working directory; the changes are mechanical
  (tail-appended slots + pure pointer memoization + one-line
  invalidate + accessor + tests mirroring the R32B/R33B
  telemetry pattern). The byte-identity claim for 369 prior
  assertions rests on the tail-append discipline.
* **R36B touches only `dtls12.nova` + `test_dtls12.nova` + docs.**
  srtp.nova / turn.nova / ice_turn.nova / sha256.nova are
  unchanged -- R36A (TURN auth, concurrent on turn.nova) cannot
  collide.
