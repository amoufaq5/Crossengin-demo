...
| Chat dispatcher | R39A | Intent dispatcher in `examples/crossengin_chat.nova` (ADR-0038 self-model query + ADR-0026 self-learning triggers): adds three reply paths AHEAD of the legacy ACK fallback so the REPL stops saying "okay" to everything. Self-identification queries ("who are you" / "what are your goals" / "how are you" / "what can you do" / "what are you doing") route to the real `selfmodel_identity` / `selfmodel_state` / `selfmodel_goals` / `selfmodel_activity` and a competence-count surface for capabilities; concept queries ("what is X" / "tell me about X" / bare "X") render the highest-belief triple from KG operator atoms as "X is a Y" / "X causes Y" / "X has Y"; inputs with `unk > 0` reply "I don't know X yet -- I'll look into it" and bump a `chat_stats_self_learning_triggers_filed` counter as `_submit_curiosity` files SLT signals. Helpers (`_intent_self_id_kind`, `_smq_extract_concept_query`, `_smq_relation_phrase`, `_smq_template_triple`, `_smq_unknown_reply`) live in `src/chat/helpers.nova` and are unit-tested via `tests/unit/test_chat_intent_dispatch.nova` (60+ assertions). |
| Learning / preprocess | R39F | Text preprocessing pipeline `src/learning/preprocess.nova` (ADR-0028 transform seam / ADR-0031): HTML strip with `<script>`/`<style>` block removal + 6 entity decodes + block-tag-to-newline + whitespace collapse, sentence splitter with `Dr.` / `Mr.` / `e.g.` / `i.e.` abbreviation guards, tokenizer (lowercase, punctuation-boundary, numeric + single-char drop), stopword filter with ~110 English defaults and `preprocess_set_stopwords` swap hook, six-pattern triple extractor (`is_a`, `has_property`, `causes`, `part_of`, `defined_as`, `is defined as`) capped at 2 triples per sentence, and a `preprocess_run(html) -> [atoms, triples]` composer with telemetry counters. Lifts the previous sed/awk pipeline out of `scripts/learn.sh` into a real NOVA module the autonomous research loop (R39D) calls in the PREPROCESSING phase. Honest scope: English-only stopwords; naive HTML strip (no CDATA, no JS-rendered DOM); high-precision low-recall triple patterns by design (extending the relation list is one anchored block per pattern). |
| Chat state persistence | R39C | File-backed save / load API for chat session state (`src/persistence/chat_state.nova`): closes the "each turn forgets" gap (scripts/chat.sh) and the cross-restart durability gap (NOVA enhancement #9 / #10 / ADR-0048 rehydration order). Text "VERSION 1" line-record format at `$HOME/.crossengin/chat_state.dat` (overridable via `CE_PERSIST_PATH`). Persists soul (name, identity, OCEAN, values), multi-KG state (per-KG atoms + caller-supplied triples), and the decision-log tail (default last 100 entries). Forward-compat: unknown record types within a known version are SKIPPED. Backward-compat: schema bumps own their migration -- a future R39C.2 v2 reader reads both. Error sentinels (`PERSIST_ERR_VERSION` / `PERSIST_ERR_NOT_FOUND` / `PERSIST_ERR_PARSE` / `PERSIST_ERR_IO`). Tests: `tests/unit/test_chat_state_persistence.nova` (88 assertions). Chat REPL wire INTENTIONALLY DEFERRED to R40 -- R39A is concurrently editing `examples/crossengin_chat.nova`. Honest gaps: text format isn't size-optimized (a 100k-atom KG lands around 10 MB; a R39C.2 binary path is documented); no atomic write yet (single open + truncate, no tmp + rename); no multi-process lock. |
| Docs / R39 architecture | R39E | Three new ADRs and a comprehensive chat usage guide describing the R39 sprint end-to-end: [`docs/adr/r39-autonomous-learning-architecture.md`](docs/adr/r39-autonomous-learning-architecture.md) (trigger -> research -> preprocess -> ingest pipeline with the diagram, performance budget, failure modes, security posture, and honest gaps incl. HTTPS deferred to R39B.2 and English-only stopwords), [`docs/adr/r39-self-identification-wiring.md`](docs/adr/r39-self-identification-wiring.md) (chat intent dispatcher and the regex pattern -> `SELFQ_*` map with priority self-ID-first rule), [`docs/adr/r39-persistence-and-rehydration.md`](docs/adr/r39-persistence-and-rehydration.md) (text v1 line-record save/load API at `$HOME/.crossengin/chat_state.dat` and the R40 chat-integration plan), and [`docs/CHAT_USAGE.md`](docs/CHAT_USAGE.md) (12-section user-facing walkthrough: quick start, web vs terminal modes, full admin-command table, teaching, self-directed learning, self-identification, inspection, persistence, troubleshooting, ADR pointers). `docs/GETTING_STARTED.md` gets a cross-reference at Section 2.7. Documents the actual landed shape of sibling R39 work including the honest gaps. No code module modified. |
